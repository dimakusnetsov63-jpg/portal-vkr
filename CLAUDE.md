...
датам). Раздел доступен **всем четырём базовым ролям** — вместе с
«Уведомлениями» единственный такой случай. Специализация (`position`) — тот же справочник
...
(`rate_card_id ... on delete cascade`). Раздел доступен всем четырём базовым
ролям, как «Адреса». Доход за смену/неделю/месяц **не хранится** — считается на
...
`20260805*`, **применены к боевой БД**, подтверждено прямой проверкой
11 августа 2026, см. `docs/database/migrations.md`) вместо статического `vacancyData.ts`
...
архивируется). Раздел доступен всем четырём базовым ролям, но **редактируют
только head и coordinator** (`portal_can('settings')` как гейт записи) — в
отличие от «Адресов»/«Ставок», где все четыре базовые роли и читают, и пишут.
...
- **Ролей шесть:** четыре базовые (`head`, `coordinator`, `manager`,
  `recruiter`) плюс `okk` («ОКК») и `marketolog` («Маркетолог»). Последние
  две заведены без единого права — их разделы назначает руководитель в
  «Настройки → Доступы». Новая роль = значение enum `portal_user_role` +
  строки в `portal_section_permissions` (отдельными миграциями: значение
  enum нельзя использовать в создавшей его транзакции) + `PORTAL_ROLES`,
  `ROLE_LABELS`, `ROLE_PERMISSIONS` в `roles.ts`.
...
6. После логического этапа — `npm run typecheck`.
7. В конце — `npm run lint`, `npm test` и `npm run build`.
...
- `typecheck` проверяет только типы.
...
- CI (`.github/workflows/ci.yml`, H-12) прогоняет все четыре обязательных
  шага на каждый `push`/`pull request` в `main` и вручную. Секретов не
  требует. Статус CI пока не блокирует прямой push в `main` — это решение о
  процессе (переход на PR + branch protection), отдельное от H-12.
