...
## Фаза C: политики переведены на VIEW/EDIT

11 августа 2026, миграции `20260811110000`…`20260811110300`, **применены к
боевой базе 11 августа 2026**. Все 50 политик переписаны: `select` спрашивает
`portal_can_view_section(<раздел>)`, `insert`/`update`/`delete` —
`portal_can_edit_section(<раздел>)`. Проектная проверка H-6 перенесена
дословно, принцип не менялся.

**Единственное изменение поведения — `project_import_configs` и
`staffing_demand_imports` получили `portal_has_project(project)`** (миграция
C4, `20260811110300`). Пробел, зафиксированный ниже как долг, закрыт:
координатор с ограниченным списком проектов больше не видит конфигурацию
парсера и историю импортов чужих проектов. `head` не затронут (bypass).

Остальные три миграции (C1–C3) поведение не меняют — у затронутых разделов
в baseline `can_edit = can_view`, а у `vacancies` разница уже закодирована
в матрице.

**Чтение `candidate_list_options` не привязано к разделу** — с
26 августа 2026 его открывает любой активный сотрудник портала
(`portal_is_active_user()`), см. миграцию `20260826100000`. До этого
требовался VIEW раздела «Кандидаты», и роль без него — например «ОКК» с
одним лишь «Контролем качества» — получала **ноль строк вместо отказа**:
все выпадающие списки формы молча оказывались пустыми. Таблица носит
историческое имя, но с TASK-009 это общепортальный справочник, который
читают шесть разделов, поэтому условие чтения не может зависеть от одного
из них. Запись осталась под `portal_can_edit_section('settings')`.

**Связка «раздел + `settings`» сохранена** у `candidate_list_options`,
`address_demand_history`, `project_import_configs` и
`staffing_demand_imports`: раздел «Адреса»/«Кандидаты» редактируют все
четыре роли, а справочники, историю и импорт — только head/coordinator.
У `vacancy_*` та же связка, наоборот, **растворена** в
`portal_can_edit_section('vacancies')` — там различие уже несёт baseline
(`can_edit = false` у manager/recruiter).

Вместе с политиками обновлены три `SECURITY DEFINER`-функции
(`portal_save_vacancy_project_tree`, `portal_duplicate_vacancy_project`,
`search_vacancy_projects`): они обходят RLS, поэтому проверка внутри тела —
основной гейт записи для редактора вакансий, и оставить её на старом
`portal_can` значило бы завести второй, расходящийся гейт.

## Фаза A новой модели доступа: политики НЕ изменены

11 августа 2026 добавлены четыре миграции (`20260811100000`…`20260811100300`,
**применены к боевой базе 11 августа 2026**), переносящие матрицу прав из кода в таблицу
...
**Ловушка фазы C, зафиксированная заранее** (учтена при реализации, см.
раздел «Фаза C» выше). У пяти таблиц запись гейтится *вторым* разделом, а не
своим собственным. Переводить их на `can_edit` «своего» раздела нельзя —
роли получат права, которых у них сейчас нет:
...
| `candidate_list_options` | **любой сотрудник** (чтение) / `settings` (запись) | ✅ | ✅ | ✅ | ❌ |
...
| `quality_checklists` | `quality` | ✅ | ✅ | ✅ | ❌ |
| `quality_checklist_groups` | `quality` | ✅ | ✅ | ✅ | **✅** |
| `quality_checklist_items` | `quality` | ✅ | ✅ | ✅ | **✅** |
| `quality_reviews` | `quality` | ✅ | ❌ | ❌ | ❌ |
| `quality_review_scores` | `quality` | ✅ | ❌ | ❌ | ❌ |
| `portal_users` | — | ❌ | ❌ | ❌ | ❌ |
| `portal_sessions` | — | ❌ | ❌ | ❌ | ❌ |
| `portal_audit_log` | — | ❌ | ❌ | ❌ | ❌ |
...
- **`quality_reviews`/`quality_review_scores` — только чтение** (TASK-013).
  Отсутствие INSERT/UPDATE-политик здесь не забывчивость, а решение:
  единственный путь записи — `portal_save_quality_review`
  (`SECURITY DEFINER`), которая пересчитывает проценты из проставленных
  баллов. Разреши прямую запись — и `total_score` стал бы полем, куда клиент
  присылает любое число независимо от баллов (ADR-006). Гранты на запись
  этим двум таблицам не выданы, так что дверь закрыта и на уровне привилегий
  Postgres, а не только политик. Так стало **не сразу**: при создании таблиц
  legacy-триггер платформы выдал обеим ролям полный набор прав, включая
  TRUNCATE (который RLS не проверяет вовсе) — устранено миграцией
  20260818100800, см. docs/database/migrations.md. Шаблоны (`quality_checklist*`), наоборот,
  редактируются обычным CRUD: производных значений у них нет. У самих
  шаблонов нет delete (архивируются), у блоков и пунктов есть — пока на них
  не сослалась ни одна проверка, дальше останавливает `on delete restrict`.
