...
| `20260802200000_login_rate_limit_revoke_anon.sql` | **НЕ ПРИМЕНЕНА.** Фаза 3: `revoke execute ... from anon` на всех трёх auth-функциях. Условие для применения выполнено (фаза 2 верифицирована в production), но применение — точка невозврата, ждёт отдельного решения |
...
`20260802200000_login_rate_limit_revoke_anon.sql` (фаза 3) — не применена.
...
`revoke execute ... from anon` на всех трёх функциях. Написана и готова к
применению — условие (подтверждённая фаза 2) выполнено. Точка невозврата,
поэтому применение — отдельное решение, не автоматическое продолжение
написания файла. До применения оба пути живы и откат бесплатен.
