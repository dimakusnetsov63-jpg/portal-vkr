...
`20260801100000_login_rate_limit.sql` **к боевой БД не применена** — см.
следующий раздел, там же порядок выката.
...
**Код готов и лежит в ветке `feat/auth-phase-2`, в `main` не влит.** Ветка —
не осторожность, а механизм: она делает «не выкатить раньше миграции»
...
Проверить руками после применения фазы 1: вход, неверный пароль, отключённая
учётка, срабатывание ограничения и точность `retry_after`, сброс счётчика
успешным входом, истёкшая сессия, выход, работа middleware на защищённых
маршрутах.

### Фаза 3 — отдельная миграция

`revoke execute ... from anon` на всех трёх функциях. Точка невозврата,
выполняется только после подтверждённой фазы 2. До неё оба пути живы и
откат бесплатен.

### Что ещё нужно сделать руками

Ретенция `portal_login_attempts` не настраивается миграцией: включение
`pg_cron` — решение уровня проекта, а не побочный эффект правки входа.
После применения:
...
Без расписания таблица растёт неограниченно — то есть одна из двух половин
C-3 останется незакрытой.
