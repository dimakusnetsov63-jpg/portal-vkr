...
8. **Номер версии проверять на занятость до создания файла.** Учёт ведётся
   по числовому префиксу, и он же первичный ключ в
   `supabase_migrations.schema_migrations`. Две миграции с одним номером —
   не косметика: на чистой базе применение падает на дубле ключа (и это
   роняет джобу `rls-tests` целиком, ещё до тестов), а на уже применённой
   базе вторая запись молча теряется в `on conflict do nothing`, после чего
   журнал миграций врёт.

   Поймано 21 августа 2026: две параллельные работы взяли
   `20260821110000` — роли ОКК/Маркетолог и возврат общего чек-листа. CI
   лёг, в журнале осталась одна запись из двух, вторая миграция числилась
   непримененной, хотя её SQL на бою уже отработал. Лечение: переименовать
   более позднюю на свободный номер и дописать запись в журнал вручную.

   Проверка стоит одну команду: `ls supabase/migrations/ | tail -5`.
9. **Новая таблица = RLS + политики + явный `GRANT`.** Три части, не две.
...
| `20260819100000_fix_samokat_local_checklist.sql` | **Применена 19 августа 2026.** Исправляет засев чек-листа «Самокат местные»: вместо текстов пунктов там оказались продублированы названия блоков (шапка этого листа устроена иначе, чем у остальных семи). Структура, шкалы и порядок не меняются — только заголовки; версия шаблона поднята на 1 |
| `20260819110000_quality_rpc_guards.sql` | **Применена 19 августа 2026.** Фаза A аудита: обновление проверяет доступ к проекту существующей строки (SEC-01, до этого проверялся только проект из payload); `kind` берётся из шаблона, а не из payload (SEC-02); завершить проверку с незаполненными пунктами нельзя (BUG-01). Схема, политики и гранты не трогаются |
| `20260819120000_seed_kc_employees.sql` | **Применена 19 августа 2026.** Фаза B4 аудита: 30 сотрудников КЦ в справочник `candidate_list_options` (`list_type = recruiter`) — список был пуст, поэтому выпадающий список в форме проверки оказался бы бесполезен. Имена собраны программно из обоих рабочих файлов и нормализованы |
| `20260820160000_quality_archive_action_cast.sql` | **Применена 20 августа 2026.** Исправление B1: действие журнала выбиралось через `CASE` из двух нетипизированных литералов, а такой `CASE` резолвится в `text` — вставка в enum-колонку падала, и архивация не работала ни у кого. Добавлено явное приведение. Поймал RLS-тест: проверка «функция существует» этого увидеть не могла, тело plpgsql типизируется при вызове |
| `20260820150000_quality_archive_actions.sql` | **Применена 20 августа 2026.** Фаза B1 аудита: события `quality_review_archived`/`quality_review_restored` в enum `portal_audit_action`. Отдельные значения, а не признак внутри «изменил»: строка журнала должна читаться сама по себе |
| `20260820150100_quality_review_archive.sql` | **Применена 20 августа 2026.** Фаза B1 (DATA-02): колонка `archived_at`, частичный индекс по активным, RPC `portal_archive_quality_review` (гейт can_edit + проект строки, пишет в журнал). **Обе отчётные функции получили `archived_at is null`** — в этом и смысл действия: архивная проверка не должна двигать средние |
| `20260826100000_list_options_portal_wide_read.sql` | Чтение `candidate_list_options` открыто любому активному сотруднику (`portal_is_active_user()`) вместо VIEW раздела «Кандидаты». Боевая жалоба: у роли «ОКК» пусты все списки формы проверки сразу — политика писалась 11 августа, когда таблица ещё была справочником кандидатов, а TASK-009 сделал её общепортальной. Точечное `or ... ('quality')` отвергнуто: чинит один случай из шести. Запись не менялась |
| `20260821150000_quality_objection_stats.sql` | **Применена 21 августа 2026.** `portal_quality_objection_stats` — возражения кандидатов: частота и средний итог проверок, где возражение встретилось. Единственный агрегат раздела, который говорит не о работе сотрудников, а о том, что говорят кандидаты: частота возражения зависит от условий вакансии и рынка, а не от качества прослушки. Пустое возражение не учитывается — «не заполнили» и «возражения не было» разные вещи |
| `20260821140000_quality_trend_and_distribution.sql` | **Применена 21 августа 2026.** Два агрегата под графики сводки: `portal_quality_report_by_month` (помесячная динамика, вид проверки не сворачивается — у прослушек и самоотказов разный смысл) и `portal_quality_score_distribution` (сколько проверок в каждом диапазоне итога; диапазоны возвращаются всегда все четыре, включая пустые — ноль честнее отсутствующего столбца). Оба повторяют правила остальных агрегатов: только завершённые, архивные исключены, право и проектная изоляция проверяются внутри запроса |
| `20260821130000_align_checklist_titles_to_source.sql` | **Применена 21 августа 2026.** 15 переименований пунктов в шаблонах «Бургер кинг Россия» и «Самокат Вахта» — формулировки приведены к рабочему файлу под импорт истории. Расхождения возникли оттого, что шаблоны копировали между проектами: у этих двух они оказались зеркальными. Меняется только текст пункта; шкала, вес, признаки и порядок не трогаются, выставленные оценки не затрагиваются (у прошлых проверок формулировка хранится снимком). Каждый UPDATE адресован по id и сверяет прежний текст — прежние формулировки видны в условии, правка отменяется чтением файла задом наперёд. На чистой базе CI не затрагивает ни строки: идентификаторы боевые |
| `20260821120000_restore_common_call_checklist.sql` | **Применена 21 августа 2026.** Возвращает общий чек-лист звонка. В редакторе у единственного общего шаблона сменили проект на «Газпромнефть» — и он не скопировался, а переехал: строка одна, у неё просто стало другое значение `project`. У остальных двадцати проектов не осталось никакого шаблона звонка. Состав копируется из архивного «Чек-лист звонка — Самокат» — того же источника, из которого общий собирали 19 августа; правки Газпромнефти (94 версии, критерии АЗС) не тронуты. Идемпотентна |
| `20260821100000_quality_group_title_deferrable.sql` | **Применена 21 августа 2026.** `quality_checklist_groups_title_key` стало откладываемым (`deferrable initially immediate`), и `portal_save_quality_checklist_tree` откладывает его внутри своей транзакции. Дерево сохраняется целиком: новые блоки вставляются раньше, чем удаляются исчезнувшие, и на полпути два блока законно носят одно название. Без отсрочки ломались обмен названиями двух блоков и замена блока одноимённым — оба с нечитаемым `duplicate key`. Перестановка порядка операций спасла бы только от второго случая. Нашла джоба `rls-tests` |
| `20260820200000_quality_checklist_conflict_code.sql` | **Применена 20 августа 2026.** Конфликт версии шаблона сигналится `P0001`, как у `portal_save_quality_review`, а не `40001`. `40001` — serialization_failure, класс «повтори транзакцию»: драйверы и пулеры повторяют такой запрос сами, хотя повторять нечего. В джобе `rls-tests` это дало тест, висевший до таймаута. Клиент ловит конфликт по тексту, не по коду |
| `20260820190000_quality_checklist_duplicate_guard.sql` | **Применена 20 августа 2026.** Понятный отказ вместо сырого `duplicate key` при попытке завести второй действующий шаблон на ту же пару «вид + проект». Частичные уникальные индексы стоят с `20260818100100`, но до редактора нарушить их было неоткуда — шаблоны заводились миграциями. Проверка стоит до вставки, а не отловом `unique_violation`: перехватывать исключение значило бы полагаться на то, что оно возникло именно по этой причине. `is not distinct from` — из-за NULL у общего шаблона. Поймал RLS-тест |
| `20260820180000_quality_checklist_tree_rpc.sql` | **Применена 20 августа 2026.** Фаза 4 TASK-013: `portal_save_quality_checklist_tree` — сохранение шаблона целиком одной транзакцией (шаблон + блоки + пункты, вставки/обновления/удаления вперемешку) с проверкой версии. Пропавшая из payload строка удаляется, только если на неё не ссылается ни одна проверка, иначе архивируется — решение принимается явной проверкой, а не отловом `on delete restrict`. Впервые у шаблонов появилась проектная изоляция: проверяется доступ и к записанному проекту, и к присланному, иначе чужой шаблон можно было бы «перенести» к себе (тот же класс дыры, что SEC-01 у проверок) |
| `20260820170000_quality_violation_zeroes.sql` | **Применена 20 августа 2026.** Обратная связь команды КЦ: справочник `qc_violation` засеян тремя значениями («Дезинформация», «Хамство», «Слив кандидата»); любое из них обнуляет итог звонка наравне с нулём по критическому пункту — проценты блоков при этом остаются настоящими; `allow_na` снят у переключателя «Было возражение?» и у всех пунктов «Прослушки самоотказов». Существующие ответы не затронуты: `is_na` на этих пунктах не было (проверено до применения). Правило обнуления живёт в двух местах, второе — `qualityScore.ts` |
| `20260820140000_quality_review_version.sql` | **Применена 20 августа 2026.** Фаза B3 аудита (BUG-05): колонка `version` у `quality_reviews` под оптимистическую блокировку |
| `20260820140100_quality_save_version_check.sql` | **Применена 20 августа 2026.** Вторая половина B3: у RPC появляется `p_expected_version`, несовпадение отвергается исключением `version_conflict` (текст взят у вакансий — клиент отличает конфликт именно по нему). Обновление **без** версии тоже отвергается: это и есть тихая перезапись. `drop`+`create` — меняется список аргументов, иначе рядом осталась бы старая перегрузка без проверки; гранты выданы заново |
| `20260820130000_quality_score_snapshot.sql` | **Применена 20 августа 2026.** Фаза B2 аудита (BUG-02): пять колонок-снимка в `quality_review_scores` — формулировка пункта, блок, названия и оба порядковых номера на момент сохранения. Backfill из текущего шаблона, затем NOT NULL и ограничения длины, плюс индекс под чтение карточки |
| `20260820130100_quality_save_snapshot.sql` | **Применена 20 августа 2026.** Вторая половина B2: RPC заполняет снимок при каждом сохранении; журнал правок берёт формулировку из снимка, а не из текущего шаблона |
| `20260820120000_quality_report_by_group.sql` | **Применена 20 августа 2026.** Фаза G1 аудита (ANL-01): `portal_quality_report_by_group` — средний процент по каждому блоку чек-листа на сотрудника за период, разворачивает снимок `group_scores` через `jsonb_each`. Тот самый разрез, который в Excel назывался «Сводная по рекрутерам» и из портала не строился вовсе. Блок без числа остаётся в выдаче с `avg_percent = null` |
| `20260820110000_quality_audit_actions.sql` | **Применена 20 августа 2026.** Фаза C5 аудита: значения `quality_review_created`/`quality_review_updated` в enum `portal_audit_action`. Отдельным файлом — новое значение enum нельзя использовать в создавшей его транзакции |
| `20260820110100_quality_review_audit.sql` | **Применена 20 августа 2026.** Фаза C5 (DATA-03): `portal_save_quality_review` пишет событие в `portal_audit_log`. У правки фиксируется «было → стало» по каждому изменённому пункту, переход статуса и сдвиг итога; баллы записываются так же, как их видит человек (`0`/`1`/`2`, `н/д`, `—`) |
| `20260820100000_quality_report_scored_count.sql` | **Применена 20 августа 2026.** Фаза C1 аудита (BUG-03): `portal_quality_report` отдаёт `scored_count` — сколько проверок дали числовой итог. Средний взвешивался на `reviews_count`, который включает строки без итога, и общий средний по разделу уезжал. `drop + create`, а не `create or replace`: состав колонок у `returns table` через replace не меняется; `EXECUTE` выдан заново, при drop гранты теряются |
| `20260819120100_quality_employee_identity.sql` | **Применена 19 августа 2026.** Фаза B4 (DATA-01): нормализация `employee_name`/`reviewer_name` при записи (края + схлопывание пробелов внутри), backfill существующих строк, два CHECK-инварианта и автосвязка `employee_user_id` по единственному совпадению ФИО с активной учёткой |
| `20260819110100_quality_text_limits.sql` | **Применена 19 августа 2026.** Фаза A аудита (SEC-03): 15 ограничений длины текстовых полей — 200 для имён и справочных значений, 500 для формулировки пункта, 1000 для заметки, 4000 для комментариев |
| `20260819100200_universal_call_checklist.sql` | **Применена 19 августа 2026.** Чек-лист звонка становится единым для всех проектов: общий шаблон (`project is null`) с 35 пунктами в 9 блоках, восемь проектных — в архив. Названия видов приведены к принятым в команде: «Прослушка КЦ», «Прослушка самоотказов». Применение через Management API прошло дважды (повтор запроса), из-за чего повторный прогон упал на уникальном индексе `quality_checklists_kind_common_key`, а шаг переименования пришлось досылать отдельно — на чистой базе (CI) файл выполняется штатно одним проходом |
| `20260819100100_align_quality_projects.sql` | **Применена 19 августа 2026.** Названия проектов в шаблонах приведены к общему справочнику портала (`candidate_list_options`, `list_type = project`): «Бургер кинг» → «Бургер кинг Россия», «Яндекс лавка» → «Яндекс Лавка», «Газпром» → «Газпромнефть», «МД» → «Мастер Деливери», «Самокат местные» → «Самокат». До этого чек-листы звонка не подбирались ни к одному проекту вовсе. «Самокат вахта» заархивирован (решение бизнеса: скрипты по должностям и типу занятости не разделяем). «Криспи» намеренно не тронут — такого проекта в справочнике нет |
| `20260821110000_portal_roles_okk_marketolog.sql` | **Применена 21 августа 2026.** Две новые роли портала: `okk` (ОКК) и `marketolog` — значения enum `portal_user_role`. Отдельным файлом от seed'а: новое значение enum нельзя использовать в создавшей его транзакции |
| `20260821110100_seed_okk_marketolog_permissions.sql` | **Применена 21 августа 2026.** 24 строки в `portal_section_permissions` (по 12 на роль), **все три флага выключены**. Права новых ролей назначает руководитель в «Настройки → Доступы»; строки заводятся потому, что `portal_admin_set_section_permission` умеет только обновлять готовую строку (`P0002`, если её нет) |
| `20260909100000_address_schedule_types.sql` | **Применена 9 сентября 2026** (`db query --linked -f`, версия дописана в `supabase_migrations.schema_migrations` вручную; типы регенерированы штатной командой и совпали с ручной правкой). `addresses` `+schedule_types text[] NOT NULL DEFAULT '{}'` с `CHECK (schedule_types <@ array[...])` на те же семь значений, что у `schedule_type`. Список актуальных графиков объекта, который собирает импорт: на одном адресе тикеты разных должностей несут разные графики, и раньше все, кроме первого, терялись. Ручное поле `schedule_type` не трогается — новое живёт рядом. Аддитивная и идемпотентная (`add column if not exists`, `drop constraint if exists` перед `add constraint`), данные не мигрируются: у существующих карточек список пуст, пока по ним не пройдёт импорт |

### Роли «ОКК» и «Маркетолог» — применены к бою 21 августа 2026

Две миграции применены по одной тем же способом, что и TASK-013:
`npx supabase db query --linked -f <файл>` (`db push` в этой среде
по-прежнему не используется). Порядок обязателен — `20260821110000`
создаёт значения enum, `20260821110100` их использует, между ними должна
пройти граница транзакции.

Проверено после применения: enum `portal_user_role` — шесть значений
(`head, coordinator, manager, recruiter, okk, marketolog`);
`portal_section_permissions` — 72 строки с `project is null`, по 12 на
роль, у `okk` и `marketolog` ноль включённых флагов, у четырёх прежних
ролей счётчики не изменились (`head` 12/12/12, `coordinator` 11 view,
`manager` 8 view / 6 edit, `recruiter` 5 view / 4 edit);
`portal_role_sections('okk')` и `('marketolog')` возвращают пустой массив,
а `portal_role_permissions('okk')` — все 12 ключей, то есть матрица
доезжает до интерфейса целиком и переключатели в «Доступах» работают.

Версии дописаны в `supabase_migrations.schema_migrations` вручную (`db
query` историю не пишет), после чего `db push --linked --dry-run` отвечает
`Remote database is up to date`. Типы регенерированы штатной командой;
единственное расхождение с ручной правкой — перенос строк в массиве
`Constants.public.Enums.portal_user_role`, состав совпал.
