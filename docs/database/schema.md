...
сохранения** — `item_title`, `group_id`, `group_title`, `group_sort_order`,
`item_sort_order` (все not null с `20260820130000`). Снимок нужен, чтобы
карточка рисовала проверку такой, какой её заполняли: правка шаблона не
должна менять ни прошлую оценку, ни её обоснование. До него карточка
обходила текущее дерево шаблона, и архивный пункт пропадал из старых
проверок вместе с ответом, а переименованный менял смысл выставленного
балла. FK на пункт — `on delete restrict`. CHECK запрещает «н/д» с
проставленным баллом.
