...
- ~~Список проектов «Ставок» не синхронизирован со списком
  `candidate_project`~~ — устранено в TASK-009: `candidates`/
  `staffing_demand`/`addresses` переведены на тот же справочник
  `candidate_list_options`, которым «Ставки» пользовались с самого начала.
