...
> **Обновление от 28 июля 2026.** Фаза 1 закрыта иначе, чем здесь
> запланировано: вместо `profiles` поверх Supabase Auth портал получил
> собственную систему пользователей — см.
> [`tasks/TASK-004-portal-auth.md`](tasks/TASK-004-portal-auth.md) и
> [`ADR-004`](architecture/decisions/ADR-004-portal-auth.md).
>
> - **ТЗ-01** — неактуально в прежнем виде: Supabase Auth не используется,
>   регистрация невозможна в принципе. Остался один пункт — выключить
>   `enable_signup` в проекте, чтобы не держать лишнюю поверхность.
> - **ТЗ-05** — сделано, но по другой схеме: `portal_users` вместо
>   `profiles`, роли `head/coordinator/manager/recruiter`. SQL ниже оставлен
>   как след рассуждения, применять его не нужно.
> - **ТЗ-06** — сделано частично: политики переписаны на `portal_can()`, то
>   есть доступ по разделам. Разграничение **по проектам** из этого ТЗ
>   по-прежнему не сделано.
> - **ТЗ-07** (автотесты RLS) и **ТЗ-08** актуальны без изменений.
