-- Обратная связь команды КЦ (20 августа 2026). Три правки правил оценки.
--
-- 1. СПРАВОЧНИК НАРУШЕНИЙ. До сих пор qc_violation был пуст намеренно: в
--    рабочих таблицах одноимённая колонка заполнялась бессистемно, словаря из
--    неё было не собрать, и список должен был завести бизнес. Он его завёл —
--    три значения: дезинформация, хамство, слив кандидата.
--
-- 2. ОБНУЛЕНИЕ ПО НАРУШЕНИЮ. Любое из трёх обнуляет итог звонка. Раньше
--    обнуление было одно — ноль по критическому пункту чек-листа; теперь
--    причин две, а эффект прежний. Проценты блоков остаются настоящими:
--    решение согласовано 20 августа. Обнули мы и блоки, сводка по блокам за
--    месяц просела бы от одного нарушения, и понять, где именно был провал,
--    стало бы нельзя — а именно на этом разрезе строятся одиннадцать метрик,
--    которые команда просит на графиках.
--
--    Правило живёт в двух местах (ADR-006): здесь и в qualityScore.ts. Правка
--    одного без другого разведёт предварительный итог в форме с сохранённым.
--
-- 3. «Н/Д» УБРАН ТАМ, ГДЕ ОН БЕССМЫСЛЕН.
--    • «Было возражение?» — переключатель блока: ответ либо «да», либо «нет»,
--      третьего состояния у вопроса нет.
--    • Все четыре пункта «Прослушки самоотказов» — там оценивают, как
--      отработано возражение, и «не применимо» означало бы, что проверять
--      было нечего. Остаются «да», «частично», «нет».
--    В остальных пунктах «Прослушки КЦ» «н/д» сохранён — команда просила
--    оставить.
--
-- Существующие ответы не затронуты: на этих пунктах ни одного is_na нет
-- (проверено на боевой базе перед миграцией, 3 проверки и 39 оценок всего).

-- 1. Справочник нарушений -----------------------------------------------

insert into public.candidate_list_options (list_type, value, sort_order) values
  ('qc_violation', 'Дезинформация', 10),
  ('qc_violation', 'Хамство', 20),
  ('qc_violation', 'Слив кандидата', 30)
on conflict do nothing;

-- 2. «Н/д» там, где его быть не должно ------------------------------------

update public.quality_checklist_items i
set allow_na = false
from public.quality_checklist_groups g, public.quality_checklists c
where i.group_id = g.id
  and g.checklist_id = c.id
  and c.archived_at is null
  and i.allow_na
  and (
    (c.kind = 'refusal')
    or (c.kind = 'call' and i.scale = 'yes_no')
  );

-- 3. Обнуление итога по нарушению -----------------------------------------

CREATE OR REPLACE FUNCTION public.portal_save_quality_review(p_review_id uuid, p_payload jsonb, p_expected_version integer DEFAULT NULL::integer)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
...
begin
  ...
  select violation into v_violation from public.quality_reviews where id = v_review_id;

  -- Обнуление итога. Две независимые причины, эффект один: ноль по
  -- критическому пункту чек-листа и зафиксированное нарушение (дезинформация,
  -- хамство, слив кандидата). Проценты блоков при этом остаются настоящими —
  -- по ним видно, что сотрудник делал хорошо, и на них строится разрез по
  -- блокам. Обнули мы и блоки — сводка за месяц просела бы от одного
  -- нарушения, а понять, где именно был провал, стало бы нельзя.
  if v_has_critical or v_violation is not null then
    v_total := 0;
  end if;
...
comment on function public.portal_save_quality_review(uuid, jsonb, integer) is
  'Сохранение проверки качества: считает проценты по блокам и итог из присланных баллов, пишет в журнал действий. Итог обнуляется двумя независимыми причинами — ноль по критическому пункту чек-листа и зафиксированное нарушение; проценты блоков в обоих случаях остаются настоящими. Права, проектная изоляция, вид проверки и версия строки проверяются внутри.';
