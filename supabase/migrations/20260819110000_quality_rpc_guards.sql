-- TASK-013, фаза A аудита: три дыры в public.portal_save_quality_review.
--
-- A1 (HIGH, SEC-01) — обновление не проверяло доступ к проекту СУЩЕСТВУЮЩЕЙ
-- строки. Проверялся только `project` из payload, то есть проект, в который
-- проверку кладут. Зная UUID чужой проверки, пользователь другого проекта
-- мог переписать её целиком — сотрудника, баллы, итог — и заодно перенести
-- к себе. Достать чужой UUID через PostgREST нельзя (SELECT закрыт тем же
-- проектным гейтом), но авторизация не должна держаться на том, что
-- идентификатор трудно узнать.
--
-- A2 (MEDIUM, SEC-02) — `kind` брался из payload с подстановкой значения
-- шаблона только при отсутствии. Клиент мог сохранить проверку с
-- `kind = 'refusal'`, заполнив её по чек-листу прослушки КЦ; сводки,
-- которые фильтруются по `kind`, после этого молча расходятся с
-- действительностью. Вид проверки — свойство шаблона, а не мнение клиента.
--
-- A3 (HIGH, BUG-01) — «завершённой» могла стать проверка, где отвечен один
-- пункт из тридцати пяти. Процент считается по отвеченным пунктам
-- (неотвеченный исключается из знаменателя наравне с «н/д»), поэтому три
-- удачных ответа давали 100% и попадали в отчётность как полноценная
-- проверка. Интерфейс показывал «осталось N», но сохранить не мешал —
-- а гейтом обязана быть база: показатель, по которому разговаривают с
-- человеком, не должен зависеть от дисциплины заполняющего.
--
-- Черновик (`status = 'draft'`) остаётся свободным намеренно: незаконченную
-- проверку нужно уметь отложить, в сводки она не попадает.
--
-- Изменения только в теле функции: схема, политики и гранты не трогаются.
...
begin
  ...
  -- A1: у существующей проверки проверяется и её нынешний проект.
  ...
  if v_review_id is null then
    insert into public.quality_reviews (
      ...
      v_checklist.id,
      v_checklist.version,
      -- A2: вид проверки — всегда вид шаблона.
      v_checklist.kind,
  ...
  -- A3: завершённая проверка обязана быть заполненной целиком.
  --
