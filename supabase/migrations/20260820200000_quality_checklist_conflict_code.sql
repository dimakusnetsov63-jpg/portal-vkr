-- Конфликт версии шаблона сигналится тем же кодом, что и у проверок.
--
-- Было `40001` — serialization_failure. Это код класса «транзакция не
-- удалась, повтори её», и обращаются с ним соответственно: драйверы,
-- пулеры и обёртки повторяют такой запрос сами. Для конфликта версии это
-- ровно неверно: повторять нечего — данные действительно изменил кто-то
-- другой, и повтор приведёт к тому же отказу. В джобе rls-tests это дало
-- тест, висевший двадцать секунд до таймаута.
--
-- `portal_save_quality_review` в том же разделе сигналит конфликт версии
-- как `P0001` (raise_exception). Две функции одного раздела не должны
-- сообщать об одном и том же состоянии по-разному.
--
-- Клиент ловит конфликт по тексту (`version_conflict`), а не по коду —
-- смена кода на нём не сказывается.
...
comment on function public.portal_save_quality_checklist_tree(uuid, jsonb, integer) is
  'Сохраняет шаблон проверки качества целиком одной транзакцией: сам шаблон, блоки и пункты, включая вставки, обновления и удаления. Пропавшая из payload строка удаляется, только если на неё не ссылается ни одна проверка, иначе архивируется. Проверяет право на раздел, доступ к проекту шаблона (и к записанному, и к присланному), единственность действующего шаблона на пару «вид + проект» и версию (конфликт версии — P0001, как у portal_save_quality_review). p_checklist_id = null создаёт шаблон.';
