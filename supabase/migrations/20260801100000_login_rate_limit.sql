...
begin
  ...
  -- Успешный вход снимает накопленные неудачи — иначе сотрудник, ошибшийся
  -- паролем десять раз и затем вошедший, упирался бы в отказ при следующей
  -- попытке в пределах окна. Это была бы та же беда «страдает владелец
  -- учётки», ради устранения которой лимит и переносился с логина на источник.
  --
  -- Чистится ТОЛЬКО пара (источник, учётная запись), а не все неудачи с этого
  -- IP. Разница существенная: снятие всех означало бы, что атакующий,
  -- владеющий одной валидной учёткой, обнуляет себе счётчик успешным входом и
  -- спокойно продолжает перебор остальных с того же адреса. Снятие неудач по
  -- собственной учётке такой возможности не даёт.
  --
  -- `is not distinct from` вместо `=`: v_ip_hash может быть NULL (источник
  -- неизвестен), а `null = null` в SQL не истинно.
  delete from public.portal_login_attempts
  where ip_hash is not distinct from v_ip_hash
    and login_hash = v_login_hash
    and not succeeded;

  -- Сама успешная попытка записывается: в лимите она не участвует (в выборке
