-- TASK-013, фаза B2 аудита (BUG-02), вторая половина: RPC заполняет снимок
-- формулировки и положения пункта при каждом сохранении.
--
-- Колонки завела предыдущая миграция (20260820130000) и заполнила уже
-- существующие строки; здесь закрывается путь на будущее — иначе первая же
-- новая проверка легла бы с пустым снимком и упала на NOT NULL.
--
-- Заодно журнал правок (C5) переходит на снимок: в записи «было → стало»
-- теперь стоит формулировка, действовавшая на момент правки, а не текущая.
-- Иначе переименование пункта задним числом меняло бы смысл старых записей
-- журнала — ровно та беда, от которой избавляет весь B2.
--
-- Остальное тело функции не менялось: гейты SEC-01/SEC-02, проверка
-- полноты (BUG-01), нормализация имени и автосвязка (B4), расчёт процентов
-- и запись событий — как были.
