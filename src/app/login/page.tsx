...
export default function LoginPage() {
  ...
  async function handleSubmit(e: FormEvent) {
    ...
    setPassword("");
    // Без router.refresh(): он удваивал запрос к `/` — тот же RSC-рендер и
    // та же проверка сессии ещё раз. Прежний кэш роутера здесь взяться
    // неоткуда: до входа `/` ни разу не рендерился (middleware уводил на
    // /login редиректом), а страница динамическая, то есть staleTime у неё
    // нулевой и replace() и так идёт на сервер за свежим payload.
    router.replace("/");
  }
