...
export async function GET() {
  ...
  try {
    ...
    // Чаще всего — не задан SUPABASE_JWT_PRIVATE_JWK, либо его содержимое не
    // проходит импорт как ключ ES256 (см. importPortalSigningKey в jwt.ts).
    // Наружу подробности не отдаём, но и молча «успех» вернуть нельзя: без
    // токена данные не загрузятся, и причину надо видеть.
