...
function ActiveSection() {
  ...
  // Именно право читать раздел, а не `visible`: раздел, скрытый из меню, но
  // разрешённый к просмотру, обязан открываться по прямой ссылке — так же,
  // как его пропускает middleware. Последний рубеж всё равно RLS: данные не
  // придут, даже если этот гейт обойти.
  if (!can(activePage)) {
    return (
      <EmptyState
