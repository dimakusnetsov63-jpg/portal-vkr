...
Карточка-дровер использует `RealCandidateDrawer.module.css` — файл переехал
в эту папку вместе с удалением mock-дровера (C-7), с которым раньше был
общим; тогда же из него убраны классы таймлайна и ленты комментариев,
принадлежавшие только mock-версии. У карточки закреплённые шапка и подвал:
кнопки «Сохранить» и «Отметить как вышедшего» лежат в `styles.foot`, вне
прокручиваемого `styles.body`.
...
- **должности** — точное совпадение (пусто = все должности); кандидат без
  должности под выбранный фильтр не попадает;
- **поиску** — подстрока по ФИО, телефону, telegram, max, external_id, городу,
  должности.

Должность (`candidates.position`) — свободный текст с подсказками из
`candidate_list_options` (`list_type = position`), как город и рекрутер.
Справочник общий для всех проектов и редактируется в Настройках.
...
- Успешно вышел = `first_shift_at` заполнено или успешная стадия (`SUCCESSFUL_STAGES`
  исключает и «Прибыл на проект», и «Уволился»).
