...
Кроме потребности импорт заполняет **условия работы**, если они есть в
файле: `metro`, `schedule_type`, `shift_type`, чекбоксы `features`
(`mapping/normalizeConditions.ts` приводит их к тому, что допускает схема).
У существующей карточки дописываются только пустые поля — ручные правки
координатора импорт не затирает.

Режимов три: «Заменить», «Добавить» и «Синхронизировать». Последний
дополнительно обнуляет `required_count` у карточек проекта, которых в файле
нет (выгрузка показывает только открытые позиции — укомплектованный объект
из неё исчезает). Карточки с `source = 'manual'` при этом не трогаются,
их количество уходит в `warnings` отчёта.

Отчёт (и dry-run, и реальный импорт) показывает не только счётчики, но и
таблицу самих объектов — `preview.ts` определяет `ImportPreviewRow`
(действие/город/должность/адрес/было/станет), `addressPlan.ts` собирает её
внутри `planAddressWrites`, `ImportDemandModal.tsx` рендерит.
