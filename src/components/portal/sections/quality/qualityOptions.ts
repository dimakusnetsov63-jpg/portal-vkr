...
/**
 * Смысл варианта ответа — «да», «частично» или «нет», независимо от шкалы.
 *
 * Нужен, чтобы покрасить уже выбранный ответ: у шкалы `0-2` балл 2 — это
 * «да», а у `0-1-2` двойка тоже «да», но появляется ещё и «частично». Без
 * общего правила пришлось бы красить по числу, и на разных шкалах один
 * цвет означал бы разное.
 */
export function answerTone(scale: QualityItemScale, value: number): "yes" | "partial" | "no" {
  if (value === 0) return "no";
  if (scale === "0-1-2" && value === 1) return "partial";
  return "yes";
}
